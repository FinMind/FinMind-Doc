#### 2026-09-19
* [Futures Minute KBar TaiwanFuturesKBar](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#futures-minute-kbar-taiwanfutureskbar-available-only-to-sponsor-members): added [Fetch all data for a specific date at once](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#taiwanfutureskbar-sponsorpro) (sponsorpro only), which downloads the minute KBars of all futures products for a day in one request. Data range 2011-01-03 ~ now
    * Python SDK: `api.taiwan_futures_kbar(date='2024-01-02', use_object=True)` (requires the latest FinMind version)
* **Query rule clarification**: [Futures tick TaiwanFuturesTick](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#futures-trading-detail-table-taiwanfuturestick-available-only-to-backer-sponsor-members), [Options tick TaiwanOptionTick](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#options-trading-detail-table-taiwanoptiontick-available-only-to-backer-sponsor-members) and [Futures Minute KBar TaiwanFuturesKBar](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#futures-minute-kbar-taiwanfutureskbar-available-only-to-sponsor-members) **require data_id** (futures / option code), consistent with the existing documentation; requests without data_id now return an error message
    * To get all products for a specific date at once, use each dataset's "Fetch all data for a specific date at once" whole-day download: [Futures tick](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#fetch-all-data-for-a-specific-date-at-once-available-only-to-sponsorpro-members), [Options tick](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#fetch-all-data-for-a-specific-date-at-once-available-only-to-sponsorpro-members_1), [Futures Minute KBar](https://finmind.github.io/en/tutor/TaiwanMarket/Derivative/#taiwanfutureskbar-sponsorpro) (sponsorpro only)
    * In the Python SDK, `taiwan_stock_tick`, `taiwan_stock_kbar`, `taiwan_futures_tick`, `taiwan_futures_kbar` and `taiwan_option_tick` now raise an error when no code is given. Please pass `stock_id` / `futures_id` / `option_id`, or use `use_object=True` to download the whole day
