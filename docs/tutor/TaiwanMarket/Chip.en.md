...
??? note "New vs old era classifications (read this first if a dealer field is 0)"
    The classification of the three institutional investors has changed over the years. **A classification has no corresponding row in eras where it did not yet exist** — this is not missing data:
...
??? note "New vs old era columns (read this first if `Dealer_buy` / `Dealer_sell` is 0)"
...
#### Active ETF Daily Holding TaiwanStockActiveETFHolding (only available for [sponsor](https://finmindtrade.com/analysis/#/Sponsor/sponsor) members)
...
- The denominator of `trading_money_pct` is the TWSE + TPEX + emerging-market individual-stock trading money of the day (ETF / ETN / beneficiary certificates / TDR and delisted instruments excluded); numerator and denominator use the same market scope
...
!!! output
    ...
    === "Schema"
        ```
        {
            ...
            trading_money_pct: float, # % of TWSE + TPEX + emerging-market individual-stock trading money
        }
        ```
