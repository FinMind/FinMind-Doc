...
- 部分資料缺失，缺失日期為：2018-12-22、2019-02-20、2019-02-21、2019-02-22；另 2019-05-16 僅有少數 ETF 的資料。
- 開啟 Async 功能，可大幅縮短資料更新時間，colab 實測 2236 檔股票，只需 3 分 40 秒即可完成下載。

??? note "TickType 內外盤標記已全面修正（2018-12-07 ~ 2023-03-10）"
    2023-03-13 以前的資料，舊版把無法判定內外盤的成交一律標記為 1（外盤），使外盤占比明顯偏高。此區間的全部交易日已重新產製，改用與 2023-03-13 之後一致的判定規則。若您先前已下載過這些日期的資料，請重新取得。

    **2021-06-22 以前的日期，約 8% ~ 10% 的成交其 TickType 為 0（無法判定）**，這是因為來源資料本身對這些成交沒有內外盤資訊，而舊版一律歸為外盤。若您以 TickType 分類統計，請將 0 單獨處理，不要併入外盤或內盤。
...
??? note "整日資料的排列順序"
    整日資料依 `stock_id`、`Time` 排序；同一時間有多筆成交時，依實際成交先後排列。可以直接依檔案順序取每檔股票的第一筆（開盤）或某個時間點前的最後一筆成交。
...
??? note "累積成交的統計範圍"
    累積成交（`TotalDealOrder`、`TotalDealVolume`、`TotalDealMoney`）只統計集中市場一般交易，**不含零股、鉅額、盤後定價、拍賣及標購**，因此收盤累計會略低於每日市場成交量，在有大量鉅額交易的日子差距會明顯放大。數量單位為張、金額單位為百萬元。

    資料間隔：2011-01-14 以前每分鐘一筆，2011-01-17 起每 15 秒，2014-02-24 起每 10 秒，2014-12-29 起每 5 秒。
...
- 部分資料缺失，缺失日期為：2019-02-20、2019-02-21、2019-02-22（這三天僅有加權指數 TAIEX 分 K，無個股資料）；另 2019-05-16 個股僅有少數 ETF 的資料。
...
??? note "13:30:00 收盤值、指數名稱沿革與已知缺漏"
    - **13:30:00 為官方收盤指數**：上市、上櫃各指數的 13:30:00 皆為當日收盤指數。上櫃指數在 13:25 進入收盤集合競價後，盤中揭示值不再變動，13:25 ~ 13:29:55 會維持同一個值，13:30:00 才是收盤指數。
    - **上櫃化學類名稱**：2007-07-02 ~ 2025-09-12 為 `ChemicalEngineering`，2025-09-15 起為 `Chemical`，兩者為同一指數，查詢跨期資料時請合併使用。
    - **已知缺漏**：2017-05-08 上櫃各指數的原始資料由 09:00:05 開始，沒有 09:00:00 這一點。
