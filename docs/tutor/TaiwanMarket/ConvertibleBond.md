...
- **月頻**資料，`date` 為該期所屬月份的 1 日（例如 2026 年 7 月的資料，`date` 為 `2026-07-01`）
- 可用 `data_id` 查詢單一可轉債（如 `11011`），或僅用日期查詢區間內全部可轉債

??? note "查無資料時請先確認 `start_date` 與資料區間"
    本資料集為月頻，`date` 只會落在每月 1 日，且資料自 2026-05-01 起提供。若查詢區間的起始日早於資料區間，或該區間內不含任何月初日期，會回傳空的結果（非錯誤）。建議把 `start_date` 設在 2026-05-01 之後、且區間至少涵蓋一個月初。
