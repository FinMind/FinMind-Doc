...
- [期貨分K TaiwanFuturesKBar](https://finmind.github.io/tutor/TaiwanMarket/Derivative/#k-taiwanfutureskbar-sponsor)
...
- [期貨夜盤三大法人買賣 TaiwanFuturesInstitutionalInvestorsAfterHours](https://finmind.github.io/tutor/TaiwanMarket/Derivative/#taiwanfuturesinstitutionalinvestorsafterhours-backersponsor)
- [選擇權夜盤三大法人買賣 TaiwanOptionInstitutionalInvestorsAfterHours](https://finmind.github.io/tutor/TaiwanMarket/Derivative/#taiwanoptioninstitutionalinvestorsafterhours-backersponsor)
...
??? note "成交量（volume）計量方式"
    ...
    **逐月份核對公式**：某到期月份（例如 202606）在同一交易時段內，

    `單式列（contract_date = 202606）volume 加總 ＋ 含該月份的價差列（如 202606/202607）volume 加總 ÷ 2 ＝ 2 × TaiwanFuturesDaily 該月份 volume − 2 × 該月份議價申報鉅額交易口數`

    - 價差列的 volume 恰為 TaiwanFuturesDaily 中該價差合約（contract_date 含兩個月份）volume 的 4 倍；每一口價差成交會分別計入兩個月份的日成交量，因此換算到單一月份時要除以 2，不能整筆加總。
    - **議價申報的鉅額交易不會出現在逐筆成交資料中，但會計入日成交量**。這類成交多發生在結算週前的轉倉（近月、次月口數相同），是結算週前後逐筆量明顯少於日線 2 倍的主要原因，並非逐筆資料缺漏。成交明細可至期交所網站「交易資訊 > 鉅額交易 > 每日成交資訊 > 議價申報成交」查詢：交易時段為「一般」者計入一般交易時段（position），「盤後」者計入該交易日的夜盤（after_market）。
