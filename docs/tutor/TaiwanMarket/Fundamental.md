...
??? note "EPS 為單季值，遇配股／分割會被追溯調整，跨季相加會對不上累計 EPS"
    本表的 `EPS`（基本每股盈餘）是**單季**數字，且各季存放的是**該季財報公布當下的原始值**。

    依 IAS 33，無償配股（股票股利）、股票分割這類不涉及對價的股數變動，必須**追溯調整所有列報期間的加權平均股數**。因此某一季發生配股後，該季財報會把先前各季的 EPS 一併往下調整，但本表不會回頭改寫已公布的舊值。結果是**同一年度內各季 EPS 的股數基準不一致，直接相加會與財報的累計 EPS 對不上**。

    以 3081（聯亞）2026 年為例，該公司 2026-07-15 除權，配發股票股利 1 元（每千股配 100 股，即無償配股 10%）。本表 2026 Q1 的 `EPS` 為 **3.44**（Q1 財報原始值，配股前股數）、2026 Q2 為 **4.62**（Q2 財報值，已是追溯調整後股數），相加為 8.06；但財報上半年累計 EPS 是 **7.75**。差異來自 Q1 的 3.44 在 Q2 財報中已被追溯調整為 `3.44 ÷ 1.1 = 3.13`，`3.13 + 4.62 = 7.75`。

    **建議做法**：需要跨季合計時，不要直接相加 `EPS`，改以 `IncomeAfterTaxes`（本期淨利）相加，再除以同一個加權平均股數。以上例計算：`(317,515 + 468,487) 千元 ÷ 約 101,420 千股 = 7.75`。

    要判斷查詢區間內是否發生過配股，可搭配 `TaiwanStockDividend` 的 `StockEarningsDistribution`（股票股利）欄位；上例該欄位值為 1.0。
