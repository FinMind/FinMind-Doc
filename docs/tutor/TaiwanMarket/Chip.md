...
??? note "數值可能於初次公布後微幅更新"
    本資料為各法人別當日的**原始成交統計**。少數交易（例如盤後定價、鉅額配對等成交）可能於初次公布後才被補計進當日統計，使某日的**買進／賣出總量事後略有增加**；當該筆成交的買、賣雙方屬同一法人別時，買進與賣出會等額增加、**買賣超（淨額）不受影響**。此屬當日原始成交的補計，並非數據錯誤，亦非錯帳更正。官方未提供固定的「最終定案時間」，請以 API 當下回傳者為最新值；若需已定案數字，建議於該交易日後一至兩個營業日再取用。

??? note "法人別分類的新舊制差異（自營商欄位為 0 請先看這裡）"
    三大法人的分類方式歷年有調整，**某一分類在尚未實施的年代不會有對應的列**，並非資料缺漏：
...
??? note "新制 / 舊制欄位差異（`Dealer_buy` / `Dealer_sell` 為 0 請先看這裡）"
...
??? note "興櫃自營商分點的 `price` 為 0 屬正常現象，非資料缺失"
    興櫃股票（`stock_id` 可用 `TaiwanStockInfo` 查為 `emerging`）採「推薦證券商報價驅動之議價交易」，與上市櫃的集中撮合不同。推薦證券商身兼造市，**以自有資金及庫存（即自營商，券商代碼結尾為 `T`）連續報出買價與賣價並履行成交義務**；其當日造市部位買賣雙向、橫跨多個價位，無單一代表成交均價，因此該分點列的 `price` 會顯示為 `0`（`buy`／`sell` 股數仍為正確值）。

    - 這是興櫃市場結構特性，**並非資料缺失或錯誤**；同一檔興櫃股票的「一般券商分點」仍有正常成交價。
...
- 部分資料缺失，缺失日期為：2023-01-11（整日無資料）, 2023-03-16, 2023-04-06, 2023-10-25, 2025-03-26
...
- 資料區間：1992-01-04 ~ now
- 資料更新時間 **星期一至六 盤後**，實際更新時間以 API 資料為主
- 計算每日交易資金在各產業鏈的分佈：以「個體公司所屬產業鏈 TaiwanStockIndustryChain」（industry / sub_industry）彙總每日個股成交，包含成分股數、成交股數、成交金額、佔上市＋上櫃＋興櫃個股成交金額比重(%)
- `sub_industry` 為空字串的列為該產業鏈總計（成分股不重複計算，**不等於**子產業列加總，因一檔股票可屬同產業鏈多個子產業）；其餘列為子產業明細
- `trading_money_pct` 分母為當日上市＋上櫃＋興櫃個股成交金額（排除 ETF／ETN／受益證券／存託憑證等非個股與已下市標的）；分子與分母採同一市場口徑
...
!!! output
    === "DataFrame"
        ...
        |---:|:-----------|:-----------|:---------------|--------------:|-----------------:|----------------:|--------------------:|
        |  0 | 2026-07-17 | 半導體     |                |           315 |       2083017591 |    693625071313 |             52.3045 |
    ...
    === "Schema"
        ```
        {
            ...
            trading_money_pct: float, # 佔當日上市＋上櫃＋興櫃個股成交金額比重(%)
        }
        ```
