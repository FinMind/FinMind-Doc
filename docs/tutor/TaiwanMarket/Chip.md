...
- 資料區間：1992-01-04 ~ now
- 資料更新時間 **星期一至六 盤後**，實際更新時間以 API 資料為主
- 計算每日交易資金在各產業鏈的分佈：以「個體公司所屬產業鏈 TaiwanStockIndustryChain」（industry / sub_industry）彙總每日個股成交，包含成分股數、成交股數、成交金額、佔上市＋上櫃個股成交金額比重(%)
- `sub_industry` 為空字串的列為該產業鏈總計（成分股不重複計算，**不等於**子產業列加總，因一檔股票可屬同產業鏈多個子產業）；其餘列為子產業明細
- `trading_money_pct` 分母為當日上市＋上櫃個股成交金額（排除興櫃，以及 ETF／ETN／受益證券／存託憑證等非個股）；分子與分母採同一市場口徑，市場範圍固定為上市＋上櫃
...
!!! output
    ...
    === "Schema"
        ```
        {
            ...
            trading_money_pct: float, # 佔當日上市＋上櫃個股成交金額比重(%)
        }
        ```
