...
* 新增 [資產交換選擇權日成交資訊 TaiwanAssetSwapOptionDaily](https://finmind.github.io/tutor/TaiwanMarket/Derivative/#taiwanassetswapoptiondaily-backersponsor)：資產交換選擇權端的每日成交統計，欄位含名目本金 `notional_amount`、成交筆數 `number_of_transactions`、最低／最高／平均權利金 `premium_lowest`／`premium_highest`／`premium_average`、合約年期 `contract_term_years`；資料區間 2011-05-03 ~ now
